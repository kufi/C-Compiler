...
LR1ItemList *createLR1Items(ArrayList *productions)
{
  LR1ItemList *list = calloc(1, sizeof(LR1ItemList));
  list->items = arrayListCreate(10, sizeof(LR1Item *));

  for(int i =0; i < arrayListCount(productions); i++)
  {
    arrayListPush(list->items, createLR1Item(arrayListGet(productions, i), END));
  }
...
LR1ItemList *closure(LR1ItemList *list, Grammar *grammar, HashMap *sets)
{
  ...
  {;
    ArrayList *production = getProductionForSymbol(grammar, getNextSymbol(item));
    ...
    for(int j = 0; j < arrayListCount(production); j++)
    {
      ...
      {
        LR1Item *ruleItem = createLR1Item(rule, arrayListGet(set->terminals, k));
...
char *createStringFromLR1Item(LR1Item *item)
{
  ...
  appendChars(&s, item->rule->productionName);
...
CC *buildCanonicalCollection(HashMap *sets, Grammar *grammar, char *goalProduction)
{
  ...
  LR1ItemList *cc0 = closure(createLR1Items(hashMapGet(grammar->productions, goalProduction)), grammar, sets);
